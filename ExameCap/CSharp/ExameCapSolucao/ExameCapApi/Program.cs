...
builder.Services.AddSwaggerGen(opcoes =>
{
    ...
    var filePath = Path.Combine(builder.Environment.ContentRootPath, @"Documents\ExameCapApi.xml");
    opcoes.IncludeXmlComments(filePath);
});

string str = builder.Configuration.GetConnectionString("ExameCapDB");
builder.Services.AddDbContext<ExameCapContexto>(options => options.UseSqlServer(str));
